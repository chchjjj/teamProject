<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ include file="/WEB-INF/seller/sellerSideBar.jsp" %>
        ...
        <body>
                        ...
                        <form id="productForm" @submit.prevent="fnSubmitProduct">
                            ...
                            <div class="image-container">
                                <div class="thumbnail-box">
                                    <img v-if="thumbnailUrl" :src="thumbnailUrl" alt="썸네일 이미지"
                                        style="max-width: 100%; max-height: 100%; object-fit: contain;"
                                        @error="handleImageError">
                                    <span v-else style="color: #999;">이미지 없음</span>
                                </div>
                                ...
                                <div class="image-upload-area">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div
                                            v-if="proNo && existingDetailImages.length > 0 && detailFiles.length === 0">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div v-if="proNo && existingLongImage && !longFile">
                                            **기존 이미지 유지 중:** {{
                                            existingLongImage.substring(existingLongImage.lastIndexOf('/') + 1) }}
                                            <button type="button" class="btn btn-secondary btn-sm"
                            ...
                            <div class="basic-info-section">
                                ...
                                <div class="category-options">
                                    ...
                                    <input type="radio" id="categoryBakery" value="쿠키" v-model="product.proType"><label
                                        for="categoryBakery">쿠키</label>
            ...
            <script>
                ...
                const directImgPath = '${product.thumbnailPath}';
                ...
                try {
                    if (productJsonStr !== 'null') {
                        ...
                        console.log('🔍 THUMBNAIL_PATH 원본 값:', parsed.THUMBNAIL_PATH);
                        console.log('🔍 DETAIL_IMAGE_PATHS 내용:', parsed.DETAIL_IMAGE_PATHS);
                        console.log('🔍 PRO_INFO 내용:', parsed.PRO_INFO);
                        
                        // 썸네일 경로 처리
                        let thumbnailPath = parsed.THUMBNAIL_PATH || null;
                        
                        // DETAIL_IMAGE_PATHS 배열에 파일이 있으면 첫 번째를 썸네일로 사용 (임시 대응)
                        if ((!thumbnailPath || thumbnailPath.endsWith('/')) && 
                            parsed.DETAIL_IMAGE_PATHS && 
                            parsed.DETAIL_IMAGE_PATHS.length > 0) {
                            console.log('⚠️ 썸네일 경로가 비정상적입니다. DETAIL_IMAGE_PATHS의 첫 번째 이미지를 사용합니다.');
                            thumbnailPath = parsed.DETAIL_IMAGE_PATHS[0];
                        } else if (thumbnailPath && (thumbnailPath.endsWith('/') || thumbnailPath.trim() === '')) {
                            console.warn('⚠️ 썸네일 경로가 비정상적이고 대체 이미지도 없습니다:', thumbnailPath);
                            thumbnailPath = null;
                        }
                        ...
                        console.log('✅ 최종 썸네일 경로:', initialProduct.thumbnailPath);
                    }
                } catch (e) {
                ...
                const app = Vue.createApp({
                    data() {
                        ...
                        // 🔥 initialProduct에서 썸네일 경로 가져오기
                        const initialThumbnail = initialProduct.thumbnailPath || null;

                        console.log('🖼️ data() 초기화 - 썸네일 경로:', initialThumbnail);
                        ...
                        return {
                            ...
                            detailFiles: [],
                            longFile: null,
                           thumbnailUrl: initialProduct.thumbnailPath || directImgPath || null,
                            ...
                            datePicker: null,
                            imgPath: initialThumbnail // 🔥 imgPath도 같은 값으로 설정
                        };
                    },
                    methods: {
    ...
    if (type === 'thumbnail') { 
        ...
        console.log('✅ 새 썸네일 선택:', this.thumbnailUrl);
    }
    else if (type === 'detail') { 
...
handleImageError(event) {
    console.error('❌ 이미지 로딩 실패:', this.thumbnailUrl);
    // 이미지 로딩 실패 시 대체 텍스트 표시
    event.target.style.display = 'none';
    event.target.parentElement.innerHTML = '<span style="color: red;">이미지 로딩 실패</span>';
},
              ...
              fnUpdate() {
                            ...
                            if (!self.product.proName || self.product.proName.trim() === '') {
                                alert('상품명을 입력해주세요.');
                                return;
                            }
                            ...
                            formData.append('proNo', self.proNo);
                            formData.append('userId', self.userId);
                            formData.append('proName', self.product.proName);
                            formData.append('price', self.product.price);
                            formData.append('deliveryFee', self.product.deliveryFee);
                            formData.append('proType', self.product.proType);
                            formData.append('lettering', self.product.lettering);
                            formData.append('status', self.product.status);
                            
                            // 불가 날짜 (JSON 문자열로)
                            formData.append('disabledDates', JSON.stringify(self.disabledDates));
                            
                            // 옵션 데이터 (JSON 문자열로)
                            formData.append('options', JSON.stringify(self.options));
                            ...
                            if (self.detailFiles && self.detailFiles.length > 0) {
                                for (let i = 0; i < self.detailFiles.length; i++) {
                                    formData.append('detailFiles', self.detailFiles[i]);
                                }
                            }
                            
                            if (self.longFile) {
                                formData.append('longFile', self.longFile);
                            }
                            
                            // 기존 이미지 유지 정보
                            formData.append('keepExistingDetail', self.detailFiles.length === 0 ? 'Y' : 'N');
                            formData.append('keepExistingLong', self.longFile ? 'N' : 'Y');
                            
                            console.log('전송할 옵션 데이터:', self.options);
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (data.result === "success" || data.message === "success") {
                                        alert("수정되었습니다!");
                    ...
                    mounted() {
                        this.initFlatpickr();
                        console.log('JSP imgPath 원본:', "${imgPath}");
                        console.log('initialProduct:', initialProduct);
                        console.log('initialProduct.thumbnailPath:', initialProduct.thumbnailPath);
                        console.log('초기 product:', this.product);
                        console.log('초기 thumbnailUrl:', this.thumbnailUrl);
                        console.log('초기 options:', this.options);
                           if (this.thumbnailUrl) {
        const img = new Image();
        img.onload = () => console.log('✅ 이미지 로드 성공:', this.thumbnailUrl);
        img.onerror = () => console.error('❌ 이미지 로드 실패:', this.thumbnailUrl);
