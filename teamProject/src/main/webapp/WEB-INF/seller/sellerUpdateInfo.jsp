...
:root {
    --color-primary: #d77b66;     /* 메인 버튼, 활성 탭 */
    --color-secondary: #b8b0aa;   /* 보조 버튼 */
    --color-text-main: #5a3921;   /* 제목, 레이블 */
    --color-bg-light: #fffaf8;    /* 입력 필드 배경 */
    --color-bg-page: #fffaf8;     /* 페이지 배경 */
    --color-bg-content: #fff;     /* 콘텐츠 배경 */
    --color-border: #f3e5dc;      /* 구분선 */
    --color-accent: #f3b8a0;      /* 강조선 */
}
...
.container {
    display: flex;
    gap: 20px;
    max-width: 1200px; /* 기존 1400px에서 1200px로 조정 (이전 요청 기준) */
    margin: 40px auto; /* 상하 40px로 조정 (이전 요청 기준) */
    padding: 0 20px;
...
/* ===== 3. 사이드바 ===== */
.sidebar {
    width: 240px; /* 250px에서 240px로 조정 (이전 요청 기준) */
    min-width: 240px;
    flex-shrink: 0;
    background: linear-gradient(180deg, #fffdf5 0%, #fff7e5 100%); /* 브라운 계열 그라데이션 */
    padding: 20px 0;
    border-radius: 12px; /* 8px에서 12px로 조정 (이전 요청 기준) */
...
.menu li a {
    ...
    color: #8c6e5a; /* 텍스트 색상 조정 */
    font-weight: 500;
    transition: background-color 0.2s;
}

.menu li a:hover {
    background-color: #f7f0e8; /* 호버 색상 조정 */
}
...
.menu li.active a {
    ...
    font-weight: bold;
    border-left: 5px solid var(--color-primary); /* 활성 색상 변경 */
}
...
.content {
    ...
    padding: 30px 40px; /* 패딩 조정 (이전 요청 기준) */
    border-radius: 12px; /* 8px에서 12px로 조정 */
...
.content h2 {
    ...
    font-weight: 700;
    color: var(--color-text-main);
    border-left: 5px solid var(--color-accent); /* 좌측 강조선 */
    padding-left: 10px;
    padding-bottom: 0;
...
.tabs {
    display: flex;
    border-bottom: 2px solid var(--color-border); /* 색상 변경 */
    margin-bottom: 20px;
}
...
.tab-button.active {
    background-color: var(--color-bg-content);
    border-bottom: 3px solid var(--color-primary); /* 활성 색상 변경 */
...
.edit-form {
    ...
    gap: 4px; /* 폼 그룹 간 간격 조정 (개별 padding으로 간격 조절) */
}
...
.form-group {
    ...
    align-items: center;
    gap: 10px;
    padding: 10px 0; /* 상하 패딩을 15px에서 10px로 조정 */
    border-bottom: 1px solid #f5eee9; /* 색상 변경 */
}
...
.form-group label {
    width: 120px;
    min-width: 120px; 
    font-weight: 600; /* bold에서 600으로 */
    color: var(--color-text-main);
    font-size: 0.95em;
    flex-shrink: 0;
    padding-right: 0; /* 20px에서 0으로 조정 */
}
...
.form-group textarea {
    ...
    border: 1px solid #e6d7ce; /* 색상 변경 */
    border-radius: 6px; /* 4px에서 6px로 조정 */
    font-size: 0.9em;
...
/* 비밀번호 그룹 스타일 재조정 */
.password-group {
    display: flex; /* form-group의 display: flex 재확인 */
    flex-wrap: nowrap; 
    align-items: center;
    gap: 10px;
}
...
.password-group input {
    flex-grow: 1;
    margin-right: 0; /* input 내부 마진 제거 */
    min-width: 100px; /* 너무 줄어들지 않도록 최소 너비 설정 */
}
...
.form-group.address-group {
    ...
    padding: 5px 0; /* 상하 패딩을 축소하여 영역 전체를 작게 만듭니다. */
    flex-direction: column; /* 레이블과 입력 필드를 분리 */
    align-items: flex-start;
    gap: 0; 
}

.form-group.address-group > label { /* 직접적인 자식 label */
    display: block; 
    padding-bottom: 0px;
    width: 100%; 
    margin-bottom: 4px; /* 레이블과 첫 번째 입력줄 사이 간격 */
    min-width: 120px;
    font-size: 0.95em;
    font-weight: 600;
    color: var(--color-text-main);
...
.address-line {
    ...
    gap: 8px; 
    width: 100%; 
    margin-bottom: 4px; /* 다음 줄과의 간격 최소화 */
    flex-wrap: nowrap; 
}
...
.address-line input#main-addr {
    flex-grow: 2;
    margin-right: 0; /* input#main-addr의 우측 마진 제거 */
}
...
.detail-addr {
    margin-bottom: 8px; /* 주소 필드와 하단 버튼 사이의 간격 */
}
...
.address-btn {
    ...
    display: block;
    margin-top: 0px;
    padding: 10px 15px; /* 일반 버튼 패딩보다 약간 크게 */
}
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data && data.info) {
                    ...
                    // (가정) 주소가 '우편번호 기본주소 상세주소' 형태로 저장되어 있다고 가정하고 분리
                    const addressParts = fullAddress.trim().split(/\s+/); // 공백을 기준으로 분리

                    if (addressParts.length >= 3) { 
                        zipCode = addressParts[0];
                        mainAddress = addressParts[1];
                        detailAddress = addressParts.slice(2).join(' '); // 나머지 상세 주소
                    } else if (addressParts.length === 2) {
                        zipCode = addressParts[0];
                        mainAddress = addressParts[1];
                    } else if (addressParts.length === 1 && fullAddress.includes('-')) {
                         // 우편번호만 있는 경우를 가정하여 첫 번째 부분을 우편번호로 지정
                         zipCode = addressParts[0];
                    } else {
                         // 주소 전체를 기본 주소로 넣는 경우
                    ...
                    // alert("회원 정보를 불러오는데 실패했습니다."); // 에러는 콘솔로만
                }
            },
            error: function (xhr, status, error) {
                console.error("회원 정보 조회 실패:", error);
                // alert("서버 통신 오류로 회원 정보를 불러올 수 없습니다."); // 에러는 콘솔로만
            }
        });
    }
    ...
    // 개별 수정 버튼들은 이제 사용하지 않으므로, 이 함수들은 제거해도 무방합니다.
    // HTML에서 해당 버튼의 onclick 속성을 제거하는 것을 권장합니다.
    ...
    function fnUpdateMemberInfo() {
        ...
        // 새 비밀번호 필드가 하나라도 입력되었다면 비밀번호 변경을 시도하는 것으로 간주
        ...
        let userAddr = '';
        if (zipcode && addrMain) {
            // 주소 정보를 MyBatis의 userAddr에 저장할 단일 문자열 형태로 만듭니다.
        ...
        const updateData = {
            ...
            email: $('#email').val(),
            phone: $('#phone').val(),
            userAddr: userAddr // 빈 문자열이더라도 전송 (MyBatis에서 처리)
        };
        
        // 비밀번호 필드에 값이 있다면, userPass 키를 추가
        if (newPw) {
            updateData.userPass = newPw; 
            updateData.currentPw = currentPw; // 서버에서 인증용으로 사용할 수 있도록 currentPw도 함께 전송
        }

        // --- AJAX 전송 ---
        $.ajax({
            url: "/member/update.dox", // 폼 액션을 무시하고 명시적 URL 사용
            type: "POST",
            data: updateData,
            success: function(response) {
                // 💡 서버 응답 구조가 {success: true/false, message: "..."} 형태라고 가정
                if (response.success) { 
