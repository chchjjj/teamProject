    ...
    <head>
        ...
        <style>
            ...
            :root {
                ...
                --light-dot-bg-color: rgba(0, 0, 0, 0.35);
                /* 살짝 투명한 검은색으로 은은한 도트 효과 */
            }
    ...
    <body>
        ...
        <%@ include file="/WEB-INF/main/header.jsp" %>
            ...
            <div id="app-container">
                <div id="app">
                    <h2 style="margin-top: 0; color: var(--primary-color); text-align: center; font-size: 30px;">주문 상세
                        내역</h2>
                    <div class="order-list">
                        <div v-for="(item, index) in groupedOrdersList" class="order-item-card">
                            <div class="item-details">
                                <div class="pro-name">{{item.proName}}</div>
                                <div>판매처: {{item.storeName}}</div>
                            </div>

                            <div class="item-meta">
                                <div class="total-price">{{item.totalPrice.toLocaleString('ko-KR')}} 원</div>
                            ...
                            <div class="info-row" v-if="orderId.length <= 0" style="margin-top:10px;">
                                ...
                                <!-- 숨겨진 실제 input (각 주문별로 id 생성) -->
                                <input type="text" :id="'deliveryDateInput_' + index" class="date-input">
                                <button class="btn delivery-date-btn" @click="openCalendar(index, item.proNo)">
                                    {{ selectedDateDisplay[index] || '픽업/배송 날짜 및 시간 선택' }}
                                </button>
                    ...
                    const app = Vue.createApp({
                        data() {
                            return {
                                ...
                                selectedDate: {}, // key: index, value: date string
                                selectedDateDisplay: {}, // 버튼에 표시할 문자열
                                datePickers: {}, // index별 flatpickr 객체  // flatpickr 객체를 저장할 변수
                        ...
                        methods: {
                            ...
                            initFlatpickr(index) {
                                const self = this;
                                const inputId = "#deliveryDateInput_" + index;

                                // 이미 생성된 경우 중복 방지
                                if (self.datePickers[index]) return;

                                self.datePickers[index] = flatpickr(inputId, {
                                    locale: "ko",
                                    enableTime: true,
                                    ...
                                    onChange(selectedDates, dateStr) {
                                        if (selectedDates.length > 0) {
                                            self.selectedDate[index] = dateStr;
                                            self.selectedDateDisplay[index] = dateStr + ' (변경)';
                                        }
                                    }
                                });
                            },

                            openCalendar(index, proNo) {
                                const self = this;

                                // 먼저 proNo 기준으로 비활성 날짜 가져오기
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        ...
                                        self.disabledDates = Array.isArray(data.list) ?
                                            data.list
                                                .filter(item => item && item.disabledDate) // undefined 제거
                                                .map(item => item.disabledDate)
                                            : [];

                                        console.log("disabledDates:", self.disabledDates);

                                        // Flatpickr 초기화 또는 기존 인스턴스에 적용
                                        if (self.datePickers[index]) {
                                            // 이미 존재하면 disable만 갱신
                                            self.datePickers[index].set('disable', self.disabledDates);
                                            self.datePickers[index].open(); // 달력 열기
                                        } else {
                                            // 생성하면서 disabledDates 반영
                                            self.initFlatpickr(index);
                                            self.datePickers[index].set('disable', self.disabledDates);
                                            self.datePickers[index].open();
                                        }
                                    },
